...
if __name__ == '__main__':
    ...
    elif config.complexity == 'nr':
        config.g_conv_dim = [128, 256, 512]
    else:
        raise ValueError("Please enter an valid model complexity from 'mr', 'hr' or 'nr'!")
        
    if config.quantum:
        config.log_dir = 'qgan-hg-{}-q{}-l{}/logs'.format(config.complexity, config.qubits, config.layer)
        config.model_save_dir = 'qgan-hg-{}-q{}-l{}/models'.format(config.complexity, config.qubits, config.layer)
        config.sample_dir = 'qgan-hg-{}-q{}-l{}/samples'.format(config.complexity, config.qubits, config.layer)
        config.result_dir = 'qgan-hg-{}-q{}-l{}/results'.format(config.complexity, config.qubits, config.layer)
